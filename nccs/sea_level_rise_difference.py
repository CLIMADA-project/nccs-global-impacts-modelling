...
def calculate_differences(
    file1, file2, columns_to_diff, output_path, historical_file, future_scenario
):
    """
...
def process_files(input_folder, columns_to_diff, output_folder):
    ...
    files = [
        ...
        and f.startswith("indirect_impacts_sea_level_rise")
    ]
    ...
    for key, group_files in file_groups.items():
        ...
        historical_file = next(
            (f for f in group_files if f["year"] == "historical"), None
        )
        future_ssp126_file = next(
            (
                f
                for f in group_files
                if f["scenario"] == "ssp126" and f["year"] == "2060"
            ),
            None,
        )
        future_ssp585_file = next(
            (
                f
                for f in group_files
                if f["scenario"] == "ssp585" and f["year"] == "2060"
            ),
            None,
        )
        ...
        historical_path = os.path.join(
            input_folder, f"{'_'.join(historical_file.values())}.csv"
        )
        future_ssp126_path = os.path.join(
            input_folder, f"{'_'.join(future_ssp126_file.values())}.csv"
        )
        future_ssp585_path = os.path.join(
            input_folder, f"{'_'.join(future_ssp585_file.values())}.csv"
        )
