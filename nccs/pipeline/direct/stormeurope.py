...
DEFAULT_DATA_DIR = Path(".", "stormeurope_hazard")


def load_winterstorm_from_storage(
    cmip_scenario, country_iso3alpha, scenario, data_path, save_dir=DEFAULT_DATA_DIR
):
    country_iso3num = str(
        int(pycountry.countries.get(alpha_3=country_iso3alpha).numeric)
    )
    filepath = f"stormeurope_hazard/stormeurope_{cmip_scenario}_{country_iso3num}.hdf5"  # replaced old statement
    return load_hazard_from_storage(filepath, data_path)


def get_hazard(scenario, country_iso3alpha, data_path, save_dir=DEFAULT_DATA_DIR):
    LOGGER.info(
        f"Trying to get storm europe hazard, for {country_iso3alpha}, {scenario}, from {data_path}."
    )
    country_iso3num = str(
        int(pycountry.countries.get(alpha_3=country_iso3alpha).numeric)
    )
    ...
    if not os.path.isfile(data_path / filename):
        load_winterstorm_from_storage(
            cmip_scenario, country_iso3alpha, scenario, data_path, save_dir
        )
        filename = f"{save_dir}/stormeurope_{cmip_scenario}_{country_iso3num}.hdf5"
    LOGGER.info(f"Loading hazard from: {data_path/filename}")
    return Hazard.from_hdf5(data_path / filename)
...
def aggregate_windstorm_by_year(haz, is_historical=False):
    if is_historical:
        ...
        year_ids = np.concatenate(
            [np.array([years[0]]), np.cumsum(dy[ix_list]) + years[0]]
        )
    ...
    ix_list = np.concatenate((np.array([0]), ix_list, np.array([len(haz.date) - 1])))
    date_list = haz.date[ix_list]
    n_years = len(date_list) - 1
    event_frequency = (
        1 / n_years
    )  # This could take a different value: we're effectively weighting all models equally with this
    ...
    out_haz = Hazard.concat(
        [
            Hazard(
                ...
                date=np.array(
                    [date_list[i]]
                ),  # First event of the season, should probably be the first day of the season but we might never use it
                intensity=sparse.csr_matrix(
                    np.max(haz.intensity[start:stop,], axis=0)  # .toarray().transpose()
                ),
            )
            for i, (start, stop, yid) in enumerate(
                zip(ix_list[:-1], ix_list[1:], year_ids)
            )
        ]
    )
    ...
    for attr_name in ["intensity", "fraction"]:
        ...
        new_matrix = sparse.csr_matrix(
            np.matrix(
                np.array(
                    [
                        np.array([matrix.A[i, new_cent_reverse_idx]])
...
def regrid(haz, regrid_centroids, threshold=50):
    ...
    if True:
        ...
        haz = change_centroids_for_real_screw_you_climada(
            haz, regrid_centroids, threshold=threshold
        )
    else:
        #
        ...
        try:
            ...
            haz.reproject_raster(
                transform=meta["transform"], width=meta["width"], height=meta["height"]
            )
...
def subset_to_countries(haz, country_iso3num_list=None, buffer=0.6):
    ...
    if not isinstance(country_iso3num_list, list):
        raise ValueError(
            "Please provide an integer or list of integers as country IDs in parameter country_iso3num_list"
        )
    ...
    haz_list = {
        country_iso3num: _subset_one_country(
            haz=haz, reg_id=country_iso3num, buffer=buffer
        )
...
def _subset_one_country(haz, reg_id, buffer):
    ...
    extent = u_coord.latlon_bounds(
        haz_country.centroids.lat, haz_country.centroids.lon, buffer=buffer
    )
...
def get_cmip6_model_list(scenario_list=None):
    ...
    ix = [
        dat["data_source"] == "CMIP6" and status == "active"
        for dat, status in zip(info_df["properties"], info_df["status"])
    ]
    ...
    if not len(versions_list) == 1:
        Warning(
            "It looks like there is now more than one version of the storm_europe data. Update the code to filter to choose the best version. For now using v1"
        )
