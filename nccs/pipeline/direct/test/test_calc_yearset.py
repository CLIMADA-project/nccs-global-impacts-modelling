...
from nccs.pipeline.direct.test.create_test_impact import (
    dummy_impact,
    dummy_impact_yearly,
)
...
class TestYearsets(unittest.TestCase):
    ...
    def test_yearset_with_poisson_process(self):
        ...
        yimp = yearset_from_imp(
            self.dummy_imp, n_sim_years=self.n_sim_years, poisson=True, seed=seed
        )
    ...
    def test_yearset_with_non_poisson_process(self):
        ...
        yimp = yearset_from_imp(
            self.dummy_imp_yearly,
            n_sim_years=self.n_sim_years,
            poisson=False,
            seed=seed,
        )
        np.testing.assert_array_almost_equal(
            self.dummy_imp_yearly.imp_mat.shape, (6, 2)
        )
        np.testing.assert_array_almost_equal(yimp.imp_mat.shape, (self.n_sim_years, 2))
    ...
    def test_yearsets_sample_consistently_when_repeated(self):
        ...
        yimp1 = yearset_from_imp(
            self.dummy_imp, n_sim_years=self.n_sim_years, poisson=True, seed=seed
        )
        yimp2 = yearset_from_imp(
            self.dummy_imp, n_sim_years=self.n_sim_years, poisson=True, seed=seed
        )
    ...
    def test_yearsets_sample_differently_without_a_seed(self):
        ...
        yimp2 = yearset_from_imp(
            self.dummy_imp, n_sim_years=self.n_sim_years, poisson=True
        )
    ...
    def test_yearsets_can_cap_exposures(self):
        ...
        self.assertTrue(
            np.max(yimp.at_event) == 0.2
        )  # Max impact of two exposures, each capped at 0.1
