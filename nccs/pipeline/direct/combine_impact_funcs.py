...
class ImpactFuncComposable(ImpactFunc):
    ...
    def compose(
        ...
        if min(impf.intensity) < 0:
            raise ValueError(
                "When composing, the second impf should have intensity values >= 0"
            )
        if enforce_unit_interval_impacts and max(impf.intensity) > 1:
            raise ValueError(
                """When composing, the second impf should have intensity values <= 1.
                             If you want to override this, set enforce_unit_interval_impacts = False"""
            )
        if not np.all(np.array(impf.paa) == 1):
            raise ValueError(
                "When composing, the second impf must have paa constant and equal to 1"
            )

        mapped_nodes_from_self = [
            (self_intensity, impf_mdd, self_paa)
            for self_intensity, impf_mdd, self_paa in zip(
                self.intensity, impf.calc_mdr(self.mdd), self.paa
            )
        ]
        ...
        mapped_nodes_from_impf = [
            (
                self_intensity,
                impf_mdd,
                np.interp(impf_intensity, self.intensity, self.paa),
            )
            for self_intensity, impf_intensity, impf_mdd in zip(
                self.invert_mdd(impf.intensity), impf.intensity, impf.mdd
            )
        ...
        df = (
            pd.concat(
                [
                    pd.DataFrame(
                        mapped_nodes_from_self, columns=["intensity", "mdd", "paa"]
                    ),
                    pd.DataFrame(
                        mapped_nodes_from_impf, columns=["intensity", "mdd", "paa"]
                    ),
                ]
            )
            .sort_values(by=["intensity", "mdd", "paa"])
            .drop_duplicates()
        )
    ...
    def invert_mdd(self, to_invert):
        ...
        if not is_monotonic(self.mdd):
            raise ValueError(
                "Can't invert an impact function where the MDD isn't monotonic"
            )
    ...
    def simplify(self):
        gradient_mdd = [
            np.nan if dx == 0 else dy / dx
            for dy, dx in zip(np.diff(self.mdd), np.diff(self.intensity))
        ]
        gradient_paa = [
            np.nan if dx == 0 else dy / dx
            for dy, dx in zip(np.diff(self.paa), np.diff(self.intensity))
        ]
    ...
    def get_step_indices(self):
        ix_intensity_steps = [
            i
            for i, (di, dm, dp) in enumerate(
                zip(np.diff(self.intensity), np.diff(self.mdd), np.diff(self.paa))
            )
...
def scale_impf(impf, translate=0, scale=1):
    if np.any(scale * impf.mdd > 1):
        raise ValueError(
            f"The chosen scaling of {scale} takes MDD above 100%. TODO: code around this"
        )
