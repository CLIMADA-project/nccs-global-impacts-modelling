...
def get_client():
    ...
    assert all([access_key_id, access_key]), (
        "S3 credentials not set. Make sure S3_ACCESS_KEY_ID and S3_ACCESS_KEY are set in your environment or the .env "
        "file."
    )

    return boto3.client(
        "s3", aws_access_key_id=access_key_id, aws_secret_access_key=access_key
    )


def download_from_s3_bucket(
    s3_filename: str, output_path: typing.Union[str, None] = None, overwrite=False
):
    """
    ...
    s3 = get_client()
    return s3.download_file(BUCKET_NAME, s3_filename, output_path)


def upload_to_s3_bucket(
    input_filepath: str, s3_filename: typing.Union[str, None] = None
):
    """
    ...
    s3 = get_client()
    return s3.upload_file(input_filepath, BUCKET_NAME, s3_filename)
        ...
        for obj in page.get("Contents", []):
            ...
            if filename.endswith("indirect/complete.csv"):
                ...
                outfile = os.path.join(
                    get_output_dir(), filename.replace("results/", "")
                )
