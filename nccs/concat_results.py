...
for io_approach in io_approaches:
    ...
    data_files = glob.glob(
        f"{get_indirect_output_dir(RUN_TITLE)}/indirect_impacts_*{io_approach}*.csv"
    )
    dfs = []
    ...
    with pd.ExcelWriter(output_path, engine="xlsxwriter") as writer:
        ...
        readme_data = {
            "Term": list(term_definitions.keys()),
            "Definition": list(term_definitions.values()),
        }
...
# Write to Excel with multiple sheets
output_path = (
    f"{get_direct_output_dir(RUN_TITLE)}/{RUN_TITLE}_direct_impacts_complete.xlsx"
)
with pd.ExcelWriter(output_path, engine="xlsxwriter") as writer:
    # Write the main data to the first sheet
...
#     data_files = glob.glob(f"{get_indirect_output_dir(RUN_TITLE)}/sea-level-difference/diff_*{io_approach}*.csv")
#     dfs = []
#     for filename in data_files:
#         df = pd.read_csv(filename)
#         df['country_of_impact_iso_a3'] = filename.split("_")[-1].split(".")[0]
...
#     }
#     # Write to Excel with multiple sheets
#     output_path = f"{get_indirect_output_dir(RUN_TITLE)}/{RUN_TITLE}_SLR_differences_complete_{io_approach}.xlsx"
#     with pd.ExcelWriter(output_path, engine='xlsxwriter') as writer:
#         # Write the main data to the first sheet
#         DS_INDIRECT_BASE.to_excel(writer, sheet_name='Data', index=False)
