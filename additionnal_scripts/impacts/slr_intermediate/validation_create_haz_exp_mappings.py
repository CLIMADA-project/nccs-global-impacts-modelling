...
exposure_metadata = exposure_metadata.sort_values(["country", "sector"]).set_index(
    ["country", "sector"]
)
exposure_metadata["res"] = np.round(exposure_metadata["res"], 5)

mapping_df = pd.DataFrame(columns=["country", "sector", "mapping_filename"]).set_index(
    ["country", "sector"]
)
...
def get_exposure_mappings(haz_df, transform):
    ...
    lons = xmin + (x_i * xres)
    lats = ymin + (y_i * yres)
    return pd.DataFrame(dict(haz_lat=y, haz_lon=x, exp_lat=lats, exp_lon=lons))
