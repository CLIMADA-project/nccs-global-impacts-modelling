...
aqueduct_metadata["scenario_rank"] = [
    scenario_rank_map[s] for s in aqueduct_metadata["scenario"]
]
aqueduct_metadata = aqueduct_metadata.sort_values(
    ["rp", "scenario_rank", "year_rank"], ascending=False
)
assert (
    len(np.unique(aqueduct_metadata["subsidence"])) == 1
)  # ranking doesn't include this yet

aqueduct_metadata = aqueduct_metadata[
    aqueduct_metadata["perc"] != 50
]  # not ready for this yet

exposure_metadata = exposure_metadata[~pd.isna(exposure_metadata["res"])]
exposure_metadata = exposure_metadata.merge(
    country_bounding_boxes, how="left", on=["country"]
)
...
if scenario_subset:
    aqueduct_metadata = aqueduct_metadata[
        aqueduct_metadata["scenario"].isin(scenario_subset)
    ]
...
for (country, sector), _ in exposure_metadata.iterrows():
    ...
    future_outfiles = [
        Path(
            output_dir,
            f"{row['scenario']}_{row['subsidence']}_{row['year']}_{row['rp']}_{sector}_{country.replace(' ', '')}.csv",
        )
...
def get_exposure_mappings(haz_df, transform):
    ...
    return pd.DataFrame(
        dict(haz_lat=y, haz_lon=x, exp_lat=lats, exp_lon=lons)
...
def process_flood_df(df, exp, country, sector, crow):
    for _, row in df.iterrows():
        ...
        if crow["lon_max"] <= 180:
            subset_lons = slr.x[
                np.logical_and(slr.x >= crow["lon_min"], slr.x <= crow["lon_max"])
            ]
        else:
            ...
            subset_lons = slr.x[
                np.logical_or(slr.x >= crow["lon_min"], slr.x <= crow["lon_max"] - 360)
            ]
        subset_lats = slr.y[
            np.logical_and(slr.y >= crow["lat_min"], slr.y <= crow["lat_max"])
        ]
        ...
        mapped = mapped.groupby(["lat", "lon"]).agg("max")
...
def process_countrysector_df(df):
    for (country, sector), crow in df[::-1].iterrows():
        ...
        process_flood_df(
            aqueduct_metadata, exp=exp, country=country, sector=sector, crow=crow
        )
...
chunked_df = [
    exposure_metadata.iloc[ix : (ix + chunk_size)]
    for ix in range(0, exposure_metadata.shape[0], chunk_size)
]
