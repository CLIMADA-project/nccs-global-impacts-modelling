...
def summarise_flooding(sector, crow, row, total_assets, output_dir, include_missing):
    ...
    if not os.path.exists(in_path):
        if verbose:
            print(
                f"... no data available for this combination. Skipping: {in_filename}"
            )
        if include_missing:
            return [
                dict(
    ...
    for threshold in flood_thresholds:
        out = dict(
            ...
            flood_depth_threshold=threshold,
            assets_affected=sum(
                exp_affected[exp_affected["depth"] >= threshold]["value"]
            ),
        )
        out["pct_assets_affected"] = (
            0
            if out["assets_affected"] == 0
            else out["assets_affected"] / out["total_assets"]
        )
...
for sector in sector_list:
    ...
    for _, crow in country_bounding_boxes.iterrows():
        ...
        for _, row in aqueduct_metadata.iterrows():
            combined_out = combined_out + summarise_flooding(
                sector, crow, row, total_assets, output_dir, include_missing
            )
