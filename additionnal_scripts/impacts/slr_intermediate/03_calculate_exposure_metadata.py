...
DATA_PATH = "configure_me_in_{__file__}"
...
total_exposures = pd.DataFrame(
    columns=["sector", "country", "value", "res", "nrow", "ncol", "transform"]
...
for sector in sector_list:
    ...
    for _, crow in country_bounding_boxes.iterrows():
        ...
        try:
            sector_exp = get_sector_exposure(sector, crow["country"], data_path=DATA_PATH)
        ...
        try:
            if "region_id" not in sector_exp.gdf.columns or (
                len(np.unique(sector_exp.gdf.region_id)) == 1
                and sector_exp.gdf.region_id.iloc[0] == 0
            ):
                print("... assigning region ids")
                sector_exp.set_region_id()
            if len(np.unique(sector_exp.gdf.region_id)) > 1:
                sector_exp.gdf = sector_exp.gdf[
                    sector_exp.gdf["region_id"] == int(country_iso3num)
                ]
            ...
            res = np.abs(
                u_coord.get_resolution(
                    sector_exp.gdf.latitude.values, sector_exp.gdf.longitude.values
                )
            ).min()
