...
def optimize(country, hazard, sectors, n_iterations=20, initial_scale=1, weight=0.47):
    ...
    for i in range(n_iterations):
        ...
        file_bi = glob.glob(f'results/{config["run_title"]}/direct/*{sectors[0]}*.csv')[
            0
        ]
        ...
        config["runs"][0]["countries"] = [country]
        run_pipeline_from_config(config)
        file_no_bi = glob.glob(
            f'results/{config["run_title"]}/direct/*{sectors[0]}*.csv'
        )[0]
    ...
    for sector in ["forestry", "mining", "manufacturing", "service", "energy"]:
        try:
            ...
            pd.DataFrame(res).to_csv(
                f"results/calib/bi-calibration-{country}-{sector}.csv"
            )
            result.append(res)
