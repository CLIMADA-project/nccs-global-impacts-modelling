...
if not os.path.exists(working_dir):
    raise NotADirectoryError(
        f"Create a working directory at {working_dir} before you start. Temporary output will go here"
    )
...
scenario_list = [
    ...
    "ssp126",
    "ssp585",
]  # See stormeurope.py for available scenarios
...
client = Client(
    cache_enabled=None
)  # Don't store downloaded files or we'll run out of memory fast
...
if False:
    for country in era5_by_country.keys():
        try:
            ...
            plt.figure.savefig(
                Path(working_dir, "temp_" + "era5" + "_" + str(country) + ".png")
            )
            plt = era5_by_country[country].centroids.plot()
            plt.figure.savefig(
                Path(
                    working_dir,
                    "temp_" + "era5" + "_centroids_" + str(country) + ".png",
                )
            )
        except:
...
for scenario in scenario_list:
    ...
    for i, gcm in enumerate(gcm_list):
        ...
        try:
            ...
            haz = stormeurope.get_raw_hazard_from_climada_api_one_gcm(
                client, gcm, scenario
            )
            ...
            for (
                ...
                haz_by_country.keys()
            ):  # TODO really this should be the countries in era5: it's an error if after regridding there are no points for it
                if country != 0:
                    filename = (
                        "temp_" + scenario + "_" + gcm + "_" + str(country) + ".hdf5"
                    )
        ...
        if False:
            ...
            plt.figure.savefig(
                Path(working_dir, "temp_" + scenario + "_" + gcm + "_" + "ALL" + ".png")
            )
    ...
    for country in era5_by_country.keys():
        if country != 0:
            ...
            haz = Hazard.concat(
                [Hazard.from_hdf5(f) for f in filename_list if os.path.isfile(f)]
            )
            filename_combined_out = (
                "stormeurope_" + scenario + "_" + str(country) + ".hdf5"
            )
            ...
            [
                os.remove(Path(working_dir, f))
                for f in filename_list
                if os.path.isfile(Path(working_dir, f))
            ]
