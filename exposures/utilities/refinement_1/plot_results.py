...
for subscore in subscores:
    ...
    df = pd.read_hdf(
        f"{project_root}/exposures/utilities/refinement_1/{subscore}/{subscore}_MRIO.h5"
    )
    colum_latitude = "latitude"
    column_longitude = "longitude"
    df[colum_latitude] = pd.to_numeric(df[colum_latitude], errors="coerce")
    ...
    ax0.set_title(
        f"{subscore} Exposure with MRIO values scaled with normalized amount {subscore} infrastucture of in M.USD"
    )
