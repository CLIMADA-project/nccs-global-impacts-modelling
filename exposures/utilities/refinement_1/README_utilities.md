...
- Step 1: Normalize the amount of infrastructure of each sector similarly to the application performed by Nirandjan et al. 2022, up until the method "Conversion 3". The normalization is performed at global scale. Code: https://github.com/CLIMADA-project/nccs-global-impacts-modelling/blob/main/utilities/step1_max_method.py
- Step 2: Convert normalized feather files of each sector to h5 files. In the steps we introduce the columns lat, lon by taking the centroid value of the polygons provided by the initial feather files. Finally, we remove rows where the amount of infrastructure is zero at the given point(/grid). Code: https://github.com/CLIMADA-project/nccs-global-impacts-modelling/blob/main/utilities/step2_convert_feather_files.py
- Step 3: Assign for each row (lat/lon location) a country ISO code that is needed by the Climada SupplyChain module. Code: https://github.com/CLIMADA-project/nccs-global-impacts-modelling/blob/main/utilities/step3_region_ID.py
- Step 4: At this step we create a new column "normalized_country", where the Subscore of given sector is normalized at country level following the maximum method (i.e. divide the values by the maximum value of the country). Code: https://github.com/CLIMADA-project/nccs-global-impacts-modelling/blob/main/utilities/step4_normalize_country_level.py
- Step 5: Assign for each datapoint a value according to the total production of each sector within a country : 
  - Assign the mriot value of each sector and multiply it with the "normalized_area" of the country. Code: https://github.com/CLIMADA-project/nccs-global-impacts-modelling/blob/main/utilities/step5_MRIO_values.py
   ```
