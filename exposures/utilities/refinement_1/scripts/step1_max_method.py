...
def set_variables():
    ...
    weight_groups = {
        ...
        "waste": {"waste_solid": 1 / 2, "waste_water": 1 / 2},
    }

    weight_subsystems = {"energy": 1, "water": 1, "waste": 1}
...
def set_paths(cisi_calculation=False):
    ...
    infra_base_path = os.path.abspath(
        os.path.join(base_path, "summary_files")
    )  # summary amount of infrastructure saved
    method_max_path = os.path.abspath(
        os.path.join(base_path, "max_method")
    )  # final files
...
def final_exposure_index(weight_subsystems, cisi_exposure, infrastructure_systems):
    ...
    exposure_index = cisi_exposure[ci_system][
        ["geometry"]
    ].copy()  # create dataframe to save data in by deep copying geometry of one of the subsystems --> consider using .set_index('index') to make index column the index of dataframe
    # exposure_index["CISI_exposure_unnormalized"] = 0

    # add columns to exposure_index df
    subscore_list = (
        []
    )  # if ci_system needs to be analyzed, save ci_system in subscore list
    for ci_system in weight_subsystems:  # loop over each CI subsystem
        ...
        if ci_system in infrastructure_systems:
            exposure_index["Subscore_{}".format(ci_system)] = cisi_exposure[ci_system][
                "Index_{}".format(ci_system)
            ] * (
                weight_subsystems[ci_system]
            )  # column for subscore # changed to weight 1 to create the subscore column
...
def cisi_overall_max_single(
    ...
    cisi_exposure = copy.deepcopy(
        cisi_exposure_base
    )  # to avoid problems later on when cisi_exposure is returned
    ...
    for ci_system in infrastructure_systems:
        for value in infrastructure_systems[ci_system]:
            cisi_exposure[ci_system][
                "Index_{}".format(value)
            ] = 0  # create column in cisi_exposure to calculate index of group
            for asset in weight_assets[ci_system][value]:
                if (
                    asset in cisi_exposure[ci_system].columns
                ):  # check whether asset in dictioniary exists in cisi exposure geodataframe
                    # conversion 1
                    if cisi_exposure[ci_system][asset].max() != 0:
                        cisi_exposure[ci_system]["Index_{}".format(asset)] = (
                            cisi_exposure[ci_system][asset]
                            / cisi_exposure[ci_system][asset].max()
                        )
                        # add column km/max km, calculate asset index
                    else:
                        cisi_exposure[ci_system]["Index_{}".format(asset)] = 0.0
                        # conversion 2
                    cisi_exposure[ci_system]["Index_{}".format(value)] += cisi_exposure[
                        ci_system
                    ]["Index_{}".format(asset)] * (
                        weight_assets[ci_system][value][asset]
                    )  # assetgroup index
                else:
                    print(
            ...
            if cisi_exposure[ci_system]["Index_{}".format(value)].max() != 0:
                cisi_exposure[ci_system]["Index_{}".format(value)] = (
                    cisi_exposure[ci_system]["Index_{}".format(value)]
                    / cisi_exposure[ci_system]["Index_{}".format(value)].max()
                )
            else:
        ...
        for value in weight_groups[ci_system]:
            if value in infrastructure_systems[ci_system]:
                cisi_exposure[ci_system]["Index_{}".format(ci_system)] += cisi_exposure[
                    ci_system
                ]["Index_{}".format(value)] * (
                    weight_groups[ci_system][value]
                )  # calculate subsystem index

        # normalization conversion 3
        if cisi_exposure[ci_system]["Index_{}".format(ci_system)].max() != 0:
            cisi_exposure[ci_system]["Index_{}".format(ci_system)] = (
                cisi_exposure[ci_system]["Index_{}".format(ci_system)]
                / cisi_exposure[ci_system]["Index_{}".format(ci_system)].max()
            )

    # # lastly, make final exposure index based on the subsystem
    exposure_index = final_exposure_index(
        weight_subsystems, cisi_exposure, infrastructure_systems
    )
...
def cisi_calculation():
    ...
    # get settings
    infrastructure_systems, weight_assets, weight_groups, weight_subsystems = (
        set_variables()
    )  # ,
    ...
    cisi_exposure_base = {
        ci_system: pd.DataFrame() for ci_system in infrastructure_systems
    ...
    for ci_system in infrastructure_systems:
        if (
            os.path.isfile(
                os.path.join(infra_base_path, "summary_{}.feather".format(ci_system))
            )
            == True
        ):  # summary_basecalcs_{}.feather
            infra_base_data = from_geofeather(
                os.path.join(infra_base_path, "summary_{}.feather".format(ci_system))
            )  # open as geofeather #summary_basecalcs_{}.feather
            cisi_exposure_base[ci_system] = infra_base_data  # save data in dictionary
        else:
            print(
                "WARNING: the following file summary_{}.feather does not exist".format(
                    ci_system
                )
            )

    ## method 1 ##
    output_overall_max = cisi_overall_max_single(
        weight_assets,
        weight_groups,
        weight_subsystems,
        infrastructure_systems,
    ...
    to_geofeather(
        (output_overall_max),
        os.path.join(method_max_path, "utilities_exposure_{}.feather".format("all")),
        crs="EPSG:4326",
    )  # save as geofeather
