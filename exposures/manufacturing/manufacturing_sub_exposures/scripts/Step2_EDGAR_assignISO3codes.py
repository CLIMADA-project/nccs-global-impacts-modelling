...
_SHAPEFILE = gpd.read_file(
    f"{project_root}/exposures/mining/refinement_1/core/TM_WORLD_BORDERS-0.3.shp"
)
...
# Change substance in filename if another one should be picked
# files = {
...
for variable, filename in files.items():
    ...
    gdf = gpd.GeoDataFrame(
        df, geometry=gpd.points_from_xy(df[column_longitude], df[column_latitude])
    )
    ...
    def get_country_vectorized_ISO_2(coordinates):
        ...
        for idx, country in tqdm(
            _SHAPEFILE.iterrows(), total=len(_SHAPEFILE), desc="Processing countries"
        ):
            ...
            iso3_codes[lbls] = country[
                "ISO3"
            ]  # Assuming 'iso3 is the column with ISO3 codes in the _SHAPEFILE
    ...
    print(
        f"Number of rows where no country could be assigned for {variable}: ",
        df1["region_id"].isnull().sum(),
    )
