...
with open(
    f"{project_root}/manufacturing/manufacturing_sub_exposures/refinement_1/raw_data_EDGAR/EDGARv6.1_CO_2011_IRO.txt",
    "r",
) as file:
    IRO_data = file.read()

with open(
    f"{project_root}/manufacturing/manufacturing_sub_exposures/refinement_1/raw_data_EDGAR/EDGARv6.1_CO_2011_NFE.txt",
    "r",
) as file:
    NFE_data = file.read()
...
concatenated_df["lon_lat_combined"] = concatenated_df.apply(
    lambda row: f"({row['lon']}, {row['lat']})", axis=1
)

# Group by 'lon_lat_combined' and sum emissions
summed_df = (
    concatenated_df.groupby("lon_lat_combined")
    .agg({"emission 2011 (tons)": "sum", "lat": "first", "lon": "first"})
    .reset_index()
)
...
output_file_path = f"{project_root}/manufacturing/manufacturing_sub_exposures/refinement_1/raw_data_EDGAR/EDGARv6.1_CO_2011_BASICMETALS_combined.txt"
...
row_for_target_value = summed_df[
    summed_df["lon_lat_combined"] == target_lon_lat_combined
]
emission_for_target_value = row_for_target_value["emission 2011 (tons)"].values
print(
    f"Emission for lon_lat_combined {target_lon_lat_combined}: {emission_for_target_value}"
)
