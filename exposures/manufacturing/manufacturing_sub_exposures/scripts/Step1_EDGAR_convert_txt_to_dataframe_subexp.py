...
for variable, filename in files.items():
    ...
    with open(
        f"{project_root}/exposures/manufacturing/manufacturing_sub_exposures/refinement_1/raw_data_EDGAR/{filename}",
        "r",
    ) as file:
        data = file.read()
    ...
    if variable == "wood" or variable == "rubber_and_plastic":
        ...
        print(
            f"emission threshold was set to {emission_threshold_scaled} as not a spector specific exposure for: {variable}"
        )
