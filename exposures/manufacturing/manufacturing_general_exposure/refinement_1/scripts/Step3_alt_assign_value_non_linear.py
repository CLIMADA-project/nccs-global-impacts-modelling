...
def get_manufacturing_exp(
    data, countries, mriot_type, mriot_year, repr_sectors, country_run
):
    ...
    for iso3_cnt in country_run:
        ...
        try:
            if country_sum_emissions != 0:
                ...
                # exponential scaling
                # # Apply exponential scaling for each grid cell's emission
                # # Exponentiate each grid cell emission and normalize by total
                # cnt_df['weighted_emissions'] = np.exp(alpha * cnt_df['emi_nox'])  # exponential scaling
                # total_weighted_emissions = cnt_df['weighted_emissions'].sum()
                # # Normalize exponentiated emissions
                # cnt_df['normalized_emissions'] = cnt_df['weighted_emissions'] / total_weighted_emissions
                # print(f"Total emissions of {iso3_cnt} NOx is {country_sum_emissions}")

                # power scaling
                cnt_df["weighted_emissions"] = np.power(cnt_df["emi_nox"], power)
                # Sum of power-scaled emissions for normalization
                total_weighted_emissions = cnt_df["weighted_emissions"].sum()

                # Normalize power-scaled emissions
                cnt_df["normalized_emissions"] = (
                    cnt_df["weighted_emissions"] / total_weighted_emissions
                )
                print(f"Total emissions of {iso3_cnt} NOx is {country_sum_emissions}")
        ...
        try:
            ...
            try:
                ROW_manufac_prod_factor = ROW_manufac_prod.loc[
                    ROW_manufac_prod["Country Code"] == iso3_cnt, "Normalized_Prod"
                ].values[0]
                ROW_country_production = (
                    glob_prod.loc["ROW"].loc[repr_sectors].sum()
                ).values[0] * ROW_manufac_prod_factor
                cnt_df["value"] = (
                    cnt_df["normalized_emissions"] * ROW_country_production
                )
            except:
                print(
...
def get_ROW_factor_WorldBank_manufac(mriot_year, IO_countries, countries):
    ...
    ROW_ManufacProd = WB_manufac[["Country Code", str(mriot_year)]][
        ~WB_manufac["Country Code"].isin(IO_countries)
    ]
