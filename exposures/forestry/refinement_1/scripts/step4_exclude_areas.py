...
import fiona
from dict_step5 import (
    DICT,
)  # new disctionary imported from forestry/step5_exposures_forestry.py given that RUS iso3 code is different in DICT_GEOFABRIK and DICT
import json
...
def read_geojson_manually(file_path):
    ...
    df = gpd.GeoDataFrame(data=data_fields, geometry=geometry)
...
df = pd.read_hdf(
    r"C:\Users\AndreaAngelidou\Documents\nccs-correntics\forestry\data\forest_exp_region_final_v3.h5"
)  ## use file from step 2 where we selected the 8 forest class areas and assigned region_id
...
gdf = gpd.GeoDataFrame(
    df, geometry=gpd.points_from_xy(df[column_longitude], df[colum_latitude])
)
gdf.crs = "EPSG:4326"
...
for iso3 in countries:
    if iso3 in dict_list:
        ...
        gdf_area = gdf_area[
            df.columns
        ]  # drop geometry column in order to save .h5 file
...
dfs = pd.concat(dfs)
dfs.to_hdf(
    f"{directory}/forest_exp_osm_step4.h5", key="data", mode="w"
)  # final file to use in step 5
