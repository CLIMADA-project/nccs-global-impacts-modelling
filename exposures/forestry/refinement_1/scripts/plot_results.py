...
print("value_sum_per_country for forestry", value_sum_per_country)
...
norm = Normalize(vmin=gdf["value"].min(), vmax=gdf["value"].mean())
sc = ax0.scatter(
    gdf["longitude"], gdf["latitude"], c=gdf["value"], cmap="cividis", norm=norm, s=0.01
)
cbar = plt.colorbar(sc, ax=ax0, label="Value")
ax0.set_xlabel("Longitude")
ax0.set_ylabel("Latitude")
ax0.set_title(
    "Forestry Exposure with MRIO values scaled by Forestry and logging production in M.USD"
)
