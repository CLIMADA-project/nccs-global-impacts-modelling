...
def get_forestry_exp_new_2(
    ...
    data = pd.read_hdf(
        "data/forest_exp_osm_step4.h5"
    )  # file from step 4 excluding national parks and protected areas
    ...
    for iso3_cnt in countries:
        ...
        try:
            cnt_df["value"] = glob_prod.loc[iso3_cnt].loc[repr_sectors].sum().values[
                0
            ] / len(cnt_df)
        except KeyError:
            LOGGER.warning(
            ...
            try:
                ROW_gdp_factor = ROW_gdp_lookup.loc[
                    ROW_gdp_lookup["Country Code"] == iso3_cnt, "Normalized_GDP"
                ].values[0]
...
df.to_hdf(
    "data/forestry_values_MRIO_avg(GDP).h5", key="data", mode="w"
)  # final file to be used in CLIMADA NCCS project
