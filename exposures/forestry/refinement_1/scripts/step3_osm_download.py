...
from climada_petals.entity.exposures.openstreetmap.osm_dataloader import (
    OSMRaw,
    OSMFileQuery,
)
...
for iso3 in iso3_list:
    ...
    PLFileQuery = OSMFileQuery(
        Path(DATA_DIR, f"{DICT_GEOFABRIK[iso3][1]}-latest.osm.pbf")
    )
    osm_queries = [
        "boundary='national_park'",
        "boundary='protected_area'",
    ]  ### key to be updated as required
