...
joined = gpd.sjoin_nearest(
    forest, deforestation, how="left", max_distance=0.03, distance_col="distance_diff"
)
joined = joined.sort_values(by="distance_diff").drop_duplicates(
    subset="geometry", keep="first"
)

joined["weight"] = 0
joined["weight"] = np.where(
    joined["value_right"] == 1, 2, 1
)  # for sensitivity analysis change weight from 2 to 10

joined = joined[
    ["latitude_left", "longitude_left", "value_left", "region_id", "weight"]
]
...
cnt_dfs.to_hdf(
    "data/forest_exp_osm_defor(v2).h5", key="data", mode="w"
)  # final file to use in step 5.2
