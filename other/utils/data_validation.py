...
def validate_from_config(config):
    ...
    for _, row in exposure_df.iterrows():
        ...
        try:
            exp = get_sector_exposure(row["sector"], country_iso3alpha, data_path=config["data_path"])
            d["exp_exists"] = True
            d["exp_has_values"] = exp.gdf.shape[0] > 0
            d["exp_nonzero"] = exp.gdf.value.max() != 0
        except Exception as e:
            d["exp_exists_error"] = "".join(
                traceback.format_exception(type(e), e, e.__traceback__)
            )
    ...
    for _, row in analysis_df.iterrows():
        ...
        try:
            ...
            d["supchain_direct_exists_error"] = "".join(
                traceback.format_exception(type(e), e, e.__traceback__)
            )
    ...
    for _, row in analysis_df.iterrows():
        ...
        try:
            ...
            d["supchain_direct_leontief_nonzero"] = (
                supchain_indirect_leontief.shape[0] > 0
                and supchain_indirect_leontief["AAPL"].max() > 0
            )
        except Exception as e:
            d["supchain_indirect_leontief_exists_error"] = "".join(
                traceback.format_exception(type(e), e, e.__traceback__)
            )
    ...
    for _, row in analysis_df.iterrows():
        ...
        try:
            ...
            d["supchain_direct_ghosh_nonzero"] = (
                supchain_indirect_ghosh.shape[0] > 0
                and supchain_indirect_ghosh["iAAPL"].max() > 0
            )
        except Exception as e:
            d["supchain_indirect_ghosh_exists_error"] = "".join(
                traceback.format_exception(type(e), e, e.__traceback__)
            )
...
if __name__ == "__main__":
    from run_configurations.config_temp_intermediate_finishing import (
        CONFIG,
