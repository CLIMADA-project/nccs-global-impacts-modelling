...
class DashboardState:
    ...
    def update_filter_options(self):
        ...
        try:
            ...
            selected_ref_year_options = sorted(
                [str(e) for e in selected_ref_year_options]
            )
            select_ref_year.options = selected_ref_year_options
            select_ref_year.value = (
                self.selected_ref_year
                if self.selected_ref_year in selected_ref_year_options
                else selected_ref_year_options[0]
            )

            selected_scenario_options = df_haz.scenario.unique()
            selected_scenario_options = sorted(
                [str(e) for e in selected_scenario_options]
            )
            select_scenario.options = selected_scenario_options
            select_scenario.value = (
                self.selected_scenario
                if self.selected_scenario in selected_scenario_options
                else selected_scenario_options[0]
            )
...
def get_barplot_source(ds):
    ...
    return dict(sectors=sectors, impact=values)
...
def on_country_selected(attr, old, new):
    ...
    new = (
        json.loads(geo_source.geojson)["features"][new[0]]["properties"]["ISO_A3"]
        if new
        else None
    )
...
def update_state():
    ...
    if CURRENT_STATE:
        CURRENT_STATE.select_hazard_type(
            "some-nonexistent-hazard"
        )  # Crash the dashboard while loading the data
    ...
    for file in glob.glob(
        f"{nccs.utils.folder_naming.get_output_dir()}/**/indirect/complete.csv",
        recursive=True,
    ):
        logging.info(f"Loading state for run {file}")
...
with open(f"{get_resources_dir()}/countries_wgs84.geojson", "r") as f:
    ...
    COUNTRIES_BY_NAME = {
        c["properties"]["ISO_A3"]: c for c in countries["features"]
    }  # ISO_A3 would way safer
...
p_countries = figure(
    ...
    tooltips=[("Name", "@ADMIN"), ("Indirect Impact", "@value")],
)
...
p_barpot = figure(
    y_range=source_barplot.data["sectors"][::-1],
    height=1300,
    width=1400,
    title=f"Expected total production impact for Switzerland",
    tools=["tap", "reset", "save"],
)

p_barpot.hbar(
    y="sectors", right="impact", height=0.9, fill_alpha=0.9, source=source_barplot
)
...
data_table_sector = DataTable(
    source=source_barplot, columns=columns, height=400, width=400
)
...
data_table_country = DataTable(
    source=country_table_source, columns=columns, height=400, width=400
)
select_hazard_type.sizing_mode = "stretch_width"

tab = Tabs(
    tabs=[
        TabPanel(child=data_table_sector, title="Sectors"),
        TabPanel(child=data_table_country, title="Countries"),
    ]
)
